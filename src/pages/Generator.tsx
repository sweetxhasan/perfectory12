              ...
              <div className="-mx-1 flex gap-3 overflow-x-auto px-1 pb-2 snap-x snap-mandatory">
                ...
                <CutPanel tone="card" stroke="url(#cut-brand-gradient)" className="w-full" contentClassName={`bg-white p-3 ${voiceId === voice.voice_id ? 'ring-2 ring-[#ec5252]/30' : ''}`}>
                  <div className="flex items-center gap-2">
                    ...
                    <CutButton type="button" variant="primary" onClick={(event) => onTogglePreview(event, voice)} className="size-8 shrink-0 px-0 py-0 text-white [&_svg]:!text-white [&_svg]:opacity-100"><Icon name={previewId === voice.voice_id ? 'pause' : 'play'} size={14} className="text-white" /></CutButton>
                  </div>
                </CutPanel>
              </div>)}
              </div>
